 ...
 * 导航手动配置（精修覆盖）
 *
 * 此文件用于手动指定导航项，优先级高于自动生成。
 * 当某个目录在此文件中配置了 overrides，则跳过自动生成逻辑。
 *
 * 支持的特殊配置：
 * 1. __home__ : 首页（特殊 key，插入到导航最前面）
 * 2. 3级嵌套菜单：在 items 中使用子菜单
 * 3. 自定义子目录显示名称：使用 displayNames 覆盖目录名
 * 4. 跳过自动生成：设置 skipAuto 为 true
 * 5. 索引页等虚拟链接：直接写入 items
 *
 * 使用说明：
 * - 添加新目录的手动配置时，key 使用 docs/ 下的目录名（如 '10.后端'）
 * - 删除自动生成项时，添加 { skipAuto: true }
 */
module.exports = {
  // 首页（特殊 key，始终放在最前面）
  __home__: { text: '首页', link: '/' },
  ...
  // 快速笔记 — 自动生成（无 overrides）
  // 注意：不加 skipAuto 则自动从目录结构生成
}
